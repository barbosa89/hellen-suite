<div class="col-xs-4 data py-4">
    <div class="spacer-lg">&nbsp;</div>
    <div class="line mb-4"></div>
    <h1 class="text-uppercase">@lang('invoices.invoice')</h1>

    <div class="data-box">
        <div class="data-separator d-block"></div>
        <h4 class="text-muted font-weight-light">No.</h4>
        <h4 class="font-weight-bold">{{ $invoice->number }}</h4>
        @if ($invoice->reservation)
            <h4><small>(@lang('invoices.reservation'))</small></h4>
        @endif
    </div>
    <div class="data-box">
        <div class="data-separator d-block"></div>
        <h4 class="text-muted font-weight-light">@lang('common.date')</h4>
        <h4 class="font-weight-bold">{{ $invoice->created_at->format('Y-m-d') }}</h4>
    </div>
    <div class="data-box">
        <h5 class="font-weight-bold text-uppercase">@lang('invoices.origin') / @lang('invoices.destination')</h5>
        <p>{{ $invoice->origin }} - {{ $invoice->destination }}</p>
    </div>
    <div class="data-box">
        <h5 class="font-weight-bold text-uppercase">@lang('payments.status')</h5>
        @if ($invoice->losses)
            <h6 class="font-weight-light">@lang('invoices.losses')</h6>
            <p>$ {{ number_format($invoice->value - $invoice->payments->sum('value'), 2, '.', ',') }}</p>
        @else
            <h6 class="font-weight-light">Total</h6>
            <p>$ {{ number_format($invoice->value, 2, ',', '.') }}</p>
            <h6 class="font-weight-light">@lang('payments.title')</h6>
            <p>$ {{ number_format($invoice->payments->sum('value'), 2, ',', '.') }}</p>
            <h6 class="font-weight-light">@lang('common.percentage')</h6>
            <p>{{ ($invoice->payments->sum('value') / $invoice->value) * 100 }}%</p>
        @endif
        <img class="img-fluid image" src="{{ empty($invoice->hotel->image) ? asset('/images/hotel.png') : asset(Storage::url($invoice->hotel->image)) }}" alt="{{ $invoice->hotel->business_name }}">
    </div>
    <div class="data-box align-text-bottom">
        <h1 class="text-center">Hotel</h1>
        <h2 class="font-weight-bold text-center">{{ $invoice->hotel->business_name }}</h2>
    </div>
</div>