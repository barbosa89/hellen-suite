    ...
    <div id="page-wrapper">
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <h2 class="text-center">@lang('common.editionOf') @lang('team.member')</h2>
                <form action="{{ route('team.update', ['id' => id_encode($member->id)]) }}" method="POST">
                    @csrf()
                    @method('PUT')

                    <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                        <label for="name">@lang('common.name'):</label>
                        <input type="text" class="form-control" name="name" id="name" value="{{ $member->name }}" required maxlength="191" placeholder="{{ trans('common.name') }}">
